...
**Admin shell:** a fixed two-column shell — a `w-56` (224px) sidebar (`bg-discord-sidebar`, `border-r border-black/20`) and a flex-1 content column on `bg-discord-base`. Below `lg` the sidebar becomes an off-canvas drawer (Discord-mobile pattern): a sticky top bar (hamburger + invader wordmark, `lg:hidden`) opens it as a `fixed` slide-over with the overlay shadow (`0 4px 12px rgba(0,0,0,0.4)`) over a `bg-black/60` scrim; navigating, Escape, or scrim tap closes it. All content columns are full-width and `min-w-0` on mobile; wide tables scroll inside their own rounded container with the row-identifying column pinned. Desktop (`lg+`) keeps the static sidebar pixel-identical. Content area centers a max-width column (`items-center`, page padding `px-6 py-8`) rather than spanning full width edge-to-edge.
